...
4. Install the **PostGIS** extension in the new `tenants_back` database. This is
   ...
   checks for the extension on every `migrate` (`prepare_database()` in
   `django.contrib.gis.db.backends.postgis`), so it must already exist before
   `migrate_schemas` runs — otherwise provisioning fails with
   `permission denied to create extension "postgis"`.

   Manually, per cluster (then verify):
   ```bash
   PGPASSWORD=$MASTER_DB_PASSWORD psql -h $DEFAULT_DB_HOST -U postgres -d tenants_back \
       -c "CREATE EXTENSION IF NOT EXISTS postgis;"

   # verify - postgis should be listed in the public schema:
   PGPASSWORD=$MASTER_DB_PASSWORD psql -h $DEFAULT_DB_HOST -U postgres -d tenants_back -c '\dx'
   ```

   Or for all clusters at once (still master credentials):
   ```bash
   for host in $DEFAULT_DB_HOST $TENANT_1_DB_HOST $TENANT_2_DB_HOST; do
   ...
   ```
   PostGIS is a per-database object, so it must exist in `tenants_back` on *every*
   cluster - the tenant apps with GIS models (e.g. `orders`) are migrated into
   each shard.
...
5. Run the bootstrap script (only once, on ONE host):
   ```bash
   sudo -u ubuntu bash -c "
       cd /home/ubuntu/mt-shards-back-test &&
