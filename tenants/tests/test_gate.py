...
class GateStatusMatrixTests(SimpleTestCase):
    ...
    def _gate(self, status: str, path: str = "/api/v1/x", schema: str = "alpha") -> int | None:
    ...
    def test_public_tenant_exempt_regardless_of_status(self) -> None:
        for st in (S.NEW, S.PENDING, S.FAILED, S.DEACTIVATED):
            self.assertIsNone(self._gate(st, "/admin/", schema="public"))

    def test_health_path_short_circuits_before_gate(self) -> None:
...
class ErrorResponseNegotiationTests(SimpleTestCase):
    ...
    def _run(self, status: str, path: str = "/x/", accept: str = "") -> Any:
    ...
    def test_deactivated_json_for_api_path(self) -> None:
        r = self._run(S.DEACTIVATED, "/api/v1/x")
        self.assertEqual(r.status_code, 403)
        self.assertEqual(r["Content-Type"], "application/json")

    def test_deactivated_html_for_browser(self) -> None:
        r = self._run(S.DEACTIVATED, "/", accept="text/html")
        self.assertEqual(r.status_code, 403)
        self.assertIn("text/html", r["Content-Type"])

    def test_not_ready_json_has_retry_after(self) -> None:
        r = self._run(S.NEW, "/api/v1/x")
        self.assertEqual(r.status_code, 503)
        self.assertEqual(r["Content-Type"], "application/json")
        self.assertEqual(r["Retry-After"], "300")

    def test_not_ready_html_for_browser_has_retry_after(self) -> None:
        r = self._run(S.NEW, "/", accept="text/html")
        self.assertEqual(r.status_code, 503)
        self.assertIn("text/html", r["Content-Type"])
        self.assertEqual(r["Retry-After"], "300")

    def test_accept_json_forces_json_on_non_api_path(self) -> None:
...
class TenantResolutionErrorTests(SimpleTestCase):
    ...
    def test_resolve_deferred_503_html(self) -> None:
        r = self._run(ResolveDeferred("known"), "/", accept="text/html")
        self.assertEqual(r.status_code, 503)
        self.assertIn("text/html", r["Content-Type"])
        self.assertEqual(r["Retry-After"], "5")

    def test_get_tenant_reraises_operational_error(self) -> None:
