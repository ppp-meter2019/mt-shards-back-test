...
# TenantCommand FIRST in the MRO: its execute() runs the USE_MULTITENANT guard,
# then super().execute() continues into UpstreamCommand's normal flow (which our
# handle() below drives). We override handle(), not execute(), so the guard is
# never bypassed.
class Command(TenantCommand, UpstreamCommand):
    help = (
